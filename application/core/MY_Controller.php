...
class MY_Controller extends CI_Controller
{
                ...
                if ($this->jwt = JWT::decode($cookie_token, $this->config->item('jwt_key'))) {
                    if ($this->jwt->ion_auth_session_hash == $this->config->item('session_hash', 'ion_auth')) {
                        if ($this->data['user'] = $this->ion_auth->user($this->jwt->user_id)->row()) {
                            ...
                            $wil = $this->ion_auth->get_users_wilayah($this->jwt->user_id);
                            if ( $wil->num_rows() == 1)
                            {
                                $this->wilayah_id = $wil->row()->wilayah_id;
                            }
                            else
                            {
    ...
    public function cek_wilayah($wilayah_id)
    {
        if ( $this->group_id == 2 )
        {
            if ( $this->wilayah_id != $wilayah_id )
            {
                $this->ion_auth->logout();
            redirect('auth/login', 'refresh');
            }
